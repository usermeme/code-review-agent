import type { Ticket, TicketProvider } from './provider.js';
...
export class ClickUpProvider implements TicketProvider {
  ...
  async fetchTicket(ref: string): Promise<Ticket | null> {
    const response = await fetch(`https://api.clickup.com/api/v2/task/${ref}?include_subtasks=true`, {
      headers: { Authorization: this.cfg.apiToken },
    });
    if (response.status === 404) return null;
    if (!response.ok) throw new Error(`ClickUp ${ref}: HTTP ${response.status}`);
    ...
    return {
      ...
      acceptanceCriteria: checklistItems.map((item) => item.name ?? '').filter(Boolean),
      subtasks: (task.subtasks ?? []).map((subtask) => ({
        title: subtask.name ?? '',
        done: subtask.status?.type === 'done' || subtask.status?.type === 'closed',
      })),
