...
import type { PlatformClient, CodeReviewPublisher } from '../vcs/interfaces/vcs-client.interface.js';
...
export class GithubProvider implements PlatformProvider {
  ...
  parseUrl(url: string): { repo: RepositoryIdentifier; resourceType: 'issue' | 'pr'; id: number } | null {
    // Expected format: https://github.com/owner/repo/pull/123 or https://github.com/owner/repo/issues/123
    const match = url.match(/^https?:\/\/(?:www\.)?github\.com\/([^/]+)\/([^/]+)\/(pull|issues)\/(\d+)/);
