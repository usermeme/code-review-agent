...
export function loadReviewSkill(dir: string): ReviewSkill {
  ...
  if (dir) {
    try {
      ...
      logger.warn({ dir, err: error }, 'code-review skill dir not readable; using embedded snapshot');
    }
    try {
      security = stripFrontmatter(readFileSync(join(dir, 'references', 'security.md'), 'utf8'));
    } catch {
