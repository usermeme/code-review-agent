...
import type { AppConfig, ModelTier } from '../config/schema.js';
...
function getTier(cfg: AppConfig, tierName: string): ModelTier {
  ...
  if (!tier) {
    throw new Error(`Unknown model tier "${tierName}"; configured tiers: ${Object.keys(cfg.models.tiers).join(', ')}`);
  }
  return tier;
}
 ...
 */
export function resolveModel(cfg: AppConfig, tierName: string): string | BaseLlm {
...
export function resolveModelInstance(cfg: AppConfig, tierName: string): BaseLlm {
