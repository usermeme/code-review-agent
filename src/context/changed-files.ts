...
import { newSideHunkRanges, type LineRange } from '../github/diff.js';
import type { PrFile } from '../github/pr.js';
...
export function expandRanges(ranges: LineRange[], context: number, totalLines: number): LineRange[] {
...
export function renderWithLineNumbers(lines: string[], range: LineRange): string {
  const out: string[] = [];
...
export function renderChangedFile(
  ...
  for (const context of [WIDE_CONTEXT_LINES, NARROW_CONTEXT_LINES]) {
    const rendered = renderSegments(path, lines, expandRanges(hunks, context, total));
    if (estimateTokens(rendered) <= maxFileTokens) return rendered;
  }
  const rendered = renderSegments(path, lines, expandRanges(hunks, NARROW_CONTEXT_LINES, total));
  return rendered.slice(0, maxFileTokens * 4) + '\n  ... [truncated for budget] ...';
}

function renderSegments(path: string, lines: string[], ranges: LineRange[]): string {
  const total = lines.length;
  const parts = [`===== FILE: ${path} (${total} lines; showing regions around the changes) =====`];
  let prevEnd = 0;
  for (const range of ranges) {
    if (range.start > prevEnd + 1) parts.push(`  ... [lines ${prevEnd + 1}-${range.start - 1} omitted] ...`);
    parts.push(renderWithLineNumbers(lines, range));
    prevEnd = range.end;
  }
  if (prevEnd < total) parts.push(`  ... [lines ${prevEnd + 1}-${total} omitted] ...`);
  return parts.join('\n');
}
...
export async function loadChangedFiles(
  ...
  for (const file of files) {
    ...
    const section = renderChangedFile(file.filename, content, file.patch, Math.min(options.maxFileTokens, budget));
