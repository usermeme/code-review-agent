...
import type {} from '../../core/redis/redis.service.js';
import { logger } from '../../core/logger/logger.service.js';
import { collectAgentDocs } from './agent-docs.service.js';
import { buildChunks, collectFiles } from './chunker.service.js';
import { cloneShallow } from './clone.service.js';
import { summarizeChunks, synthesize } from './summarize.service.js';
import type { RepoContextCache } from './repo-context-cache.service.js';
...
export class RepoContextBuilder {
  ...
  async getOrBuild(params: {
    ...
    if (fresh) {
      logger.info(
        { repo: params.repo, builtAt: fresh.builtAt },
        'repo context cache hit',
      );
      return fresh;
    }
    ...
    if (!lockToken) {
      ...
      throw new Error(
        `Repo context for ${params.repo} is being built by another run; retry shortly`,
      );
    }
  ...
  private async build(params: {
    ...
    logger.info(
      { repo: params.repo, ref: params.ref },
    ...
    try {
      ...
      const chunkSummaries = await summarizeChunks(
        this.llm,
        chunks,
        this.options.summaryConcurrency,
      );
