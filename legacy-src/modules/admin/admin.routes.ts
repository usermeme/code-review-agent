...
export function adminRoutes(services: Services): FastifyPluginCallback {
  return (app, _opts, done) => {
    app.post<{ Params: BackfillParams; Body: BackfillBody }>(
      '/backfill/:owner/:repo',
      async (req, reply) => {
        const adminToken = process.env['ADMIN_TOKEN'];
        if (!adminToken) {
          logger.warn(
            'rejecting /admin/backfill: ADMIN_TOKEN is not configured',
          );
          return reply
            .code(503)
            .send({
              error: 'admin endpoint disabled: ADMIN_TOKEN not configured',
            });
        }
        ...
        try {
          ...
          if (!installationId) {
            return reply
              .code(400)
              .send({ error: 'installationId is required' });
          }
