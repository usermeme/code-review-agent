...
const verdictResponseSchema: Schema = {
  ...
  properties: {
    ...
    correctedStartLine: {
      type: Type.INTEGER,
      description:
        'Only when confirming a real finding whose startLine points at the wrong place',
    },
    correctedEndLine: {
      type: Type.INTEGER,
      description:
        'Only when confirming a real finding whose endLine points at the wrong place',
    },
  },
...
async function verifyOne(
  ...
  const start = Math.max(
    1,
    Math.min(finding.startLine, lines.length) - CONTEXT_LINES,
  );
  const end = Math.min(
    lines.length,
  ...
  try {
    ...
    logger.warn(
      { path: finding.path, title: finding.title, err: error },
      'verification call failed; keeping finding',
    );
    return { keep: true, reason: 'verification unavailable' };
  }
}

function applyCorrectedLines(
  finding: Finding,
  verdict: Verdict,
  totalLines: number,
): void {
