...
import {
  fetchLinkedTickets,
  type TicketProvider,
} from '../../integrations/ticket/ticket.service.js';
...
import type {
  ...
  PrDiff,
} from '../../integrations/vcs/types/vcs.types.js';
...
export async function runReview(
  ...
  const prDiffString = files
    ...
    .map(
      (f) =>
        `diff --git a/${f.previousFilename || f.filename} b/${f.filename}\n--- a/${f.previousFilename || f.filename}\n+++ b/${f.filename}\n${f.patch}`,
    )
    .join('\n');

  const tickets = await fetchLinkedTickets(
    deps.ticketProviders,
    [pr.title, pr.body, pr.headSha].join('\n'),
  );
  ...
  try {
    const changedFiles = checkout
      ? await renderChangedFiles(checkout, repoFull, files, cfg)
      : '';
    logger.info(
      { repo: repoFull, pr: request.prNumber, tickets: tickets.length },
      'starting review',
    );
    ...
    const orchestrator = createOrchestrator(
      cfg,
      {
        ...
        getDiscussion: createGetDiscussionTool(
          deps.discussionStore,
          repoFull,
          cfg.discussions.searchLimit,
        ),
        ...
        repoFiles: checkout
          ? [
              createReadFileTool(checkout.dir),
              createSearchRepoTool(checkout.dir),
            ]
          : [],
      },
...
async function runUntilPlan(
  ...
  for (const prompt of prompts) {
    ...
    logger.warn(
      { sessionId },
      'orchestrator output failed to parse as ReviewPlan; retrying',
    );
  }
  throw new Error(
    `Orchestrator never produced a valid ReviewPlan; last output:\n${lastText.slice(0, 2000)}`,
  );
}
