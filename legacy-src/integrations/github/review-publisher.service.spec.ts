...
} from './review-publisher.service.js';
...
function fakeOctokit(createReview: ReturnType<typeof vi.fn>): Octokit {
  return {
    ...
    paginate: vi
      .fn()
      .mockResolvedValue([{ filename: 'src/a.ts', patch: PATCH }]),
...
describe('GithubReviewPublisher', () => {
  it('republishes with all findings in the summary when inline comments are rejected (422)', async () => {
    ...
    expect(
      (createReview.mock.calls[0]![0] as { comments?: unknown[] }).comments,
    ).toHaveLength(1);
    const retry = createReview.mock.calls[1]![0] as {
      comments?: unknown[];
      body: string;
    };
