...
function systemText(systemInstruction: unknown): string | undefined {
  ...
  if (Array.isArray(systemInstruction)) {
    parts = systemInstruction.map((p: unknown) =>
      typeof p === 'string' ? { text: p } : (p as Part),
    );
  } else {
 ...
 */
export function contentsToMessages(
  contents: Content[],
...
export function toAnthropicRequest(
  ...
  const request: Anthropic.MessageCreateParamsNonStreaming = {
    model: options.model,
    max_tokens:
      llmRequest.config?.maxOutputTokens ?? options.maxTokens ?? 16_000,
...
export function fromAnthropicMessage(message: Anthropic.Message): LlmResponse {
  ...
  return {
    ...
    finishReason: message.stop_reason
      ? STOP_REASON_MAP[message.stop_reason]
      : undefined,
    usageMetadata: {
