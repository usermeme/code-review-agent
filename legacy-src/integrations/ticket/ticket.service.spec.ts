...
import { JiraProvider, flattenAdf } from './providers/jira.provider.js';
...
describe('JiraProvider.extractRefs', () => {
  ...
  it('finds issue keys and browse URLs, deduplicated', () => {
    const refs = provider.extractRefs(
      'Fixes DPL-123 and DPL-123, see https://acme.atlassian.net/browse/CORE-9',
    );
...
describe('ClickUpProvider.extractRefs', () => {
  ...
  it('finds task URLs and CU-prefixed refs', () => {
    const refs = provider.extractRefs(
      'See https://app.clickup.com/t/abc123 and #CU-def456',
    );
