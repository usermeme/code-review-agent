...
import {
  ...
  toAnthropicRequest,
} from '../../src/models/claude-translate.js';
...
describe('contentsToMessages', () => {
  ...
  it('round-trips tool calls with explicit ids', () => {
    const messages = contentsToMessages([
      { role: 'model', parts: [{ functionCall: { id: 'toolu_1', name: 'getRepoContext', args: {} } }] },
      {
        role: 'user',
        parts: [{ functionResponse: { id: 'toolu_1', name: 'getRepoContext', response: { ok: true } } }],
      },
    ]);
    const toolUse = (messages[0]!.content as { type: string; id?: string }[])[0]!;
  ...
  it('matches tool results to generated ids by function name when ids are missing', () => {
    const messages = contentsToMessages([
      ...
      { role: 'user', parts: [{ functionResponse: { name: 'search', response: { hits: [] } } }] },
    ]);
...
describe('fromAnthropicMessage', () => {
  it('maps text, tool_use, usage and stop reason', () => {
    ...
    expect(response.content?.parts?.[1]?.functionCall).toMatchObject({ id: 'toolu_9', name: 'lookup' });
