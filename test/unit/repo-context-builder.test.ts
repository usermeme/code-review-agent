...
import { RepoContextCache } from '../../src/cache/redis.js';
import { RepoContextBuilder, type RepoContextDoc } from '../../src/context/repo-context-builder.js';
...
function doc(builtAt: string): RepoContextDoc {
  return { sections: { architecture: 'arch' }, headSha: 'aaa', builtAt, model: 'm' };
}

function setup() {
  const redis = new FakeRedis();
  const cache = new RepoContextCache(redis as unknown as Redis, 3600, MAX_AGE_SECONDS);
...
const UNBUILDABLE = { repo: 'o/r', ref: 'main', cloneUrl: 'file:///nonexistent-repo' };
...
describe('RepoContextBuilder.getOrBuild', () => {
  ...
  it('rebuilds once the latest doc is past max age', async () => {
    ...
    await cache.set('o/r', 'aaa', doc(new Date(Date.now() - 2 * MAX_AGE_SECONDS * 1000).toISOString()));
  ...
  it('throws when another run is building and nothing is cached yet', async () => {
    const { cache, builder } = setup();
    expect(await cache.tryAcquireBuildLock('o/r')).toBeTruthy();
    await expect(builder.getOrBuild(UNBUILDABLE)).rejects.toThrow(/being built by another run/);
  });
});
