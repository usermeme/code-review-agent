-- Bot findings carry no GitHub id, and Postgres unique constraints ignore
-- NULLs, so ON CONFLICT (github_id) never fired for them and every re-run of
...
WHERE dup.github_id IS NULL
  AND keep.github_id IS NULL
  AND dup.id > keep.id
...
CREATE UNIQUE INDEX IF NOT EXISTS discussions_no_github_id_dedup_idx
