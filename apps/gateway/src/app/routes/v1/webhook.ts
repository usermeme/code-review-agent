...
export default async function (fastify: FastifyInstance) {
  fastify.post(
    '/webhook',
    async function (request: FastifyRequest<{ Body: WebhookBody }>, reply) {
      const event =
        request.headers['x-github-event'] || request.headers['x-event-type'];
      ...
      try {
        if (
          ...
          if (!payload.pull_request) {
            return reply
              .code(400)
              .send({ error: 'Missing pull_request in payload' });
          }
          ...
          if (payload.ref === defaultBranchRef) {
            ...
            fastify.log.info(
              `Triggered context-builder for ${repo} on branch ${payload.repository.default_branch}`,
            );
        ...
        return reply
          .code(500)
          .send({ error: 'Internal Server Error processing webhook' });
      }
    },
  );
}
