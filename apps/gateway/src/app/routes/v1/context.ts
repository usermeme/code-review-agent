...
export default async function (fastify: FastifyInstance) {
  fastify.post(
    '/context',
    async function (request: FastifyRequest<{ Body: ContextBody }>, reply) {
      const body = request.body;

      if (!body.repo || !body.sections) {
        return reply
          .code(400)
          .send({ error: 'Missing required fields: repo, sections' });
      }
  ...
  fastify.get(
    '/context',
    async function (
      request: FastifyRequest<{ Querystring: { repo: string } }>,
      reply,
    ) {
